...
DataAxis.prototype._redrawLabel = function (y, text, orientation, className, characterHeight) {
  ...
  text += '';
  var textLength = text.length > 1 ? text.length - 1 : text.length;

  var largestWidth = Math.max(this.props.majorCharWidth, this.props.minorCharWidth);
  var newLabelWidth = textLength * largestWidth;
  if (this.maxLabelSize < newLabelWidth) {
    this.maxLabelSize = newLabelWidth;
  }
};
